...
public class TripTest {
    ...
    private List<Trip.Leg> getDummyLegsForChanges(Integer mode, Integer changes) {
        ...
        Stop departureStop = new Stop(from, null, null, Timestamp.fromDateAndNetworkOffset(new Date(42)), null);
        Stop arrivalStop = new Stop(to, Timestamp.fromDateAndNetworkOffset(new Date(43)), null, null, null);
        ...
        case 1:
            ...
            for (int i = 0; i < changes + 1; i++) {
                legs.add(
                        new Trip.Individual(Trip.Individual.Type.BIKE, from, Timestamp.fromDateAndNetworkOffset(new Date(42)), to, Timestamp.fromDateAndNetworkOffset(new Date(43)), null, 42));
            }
            break;
        case 2:
            // mixed
            for (int i = 0; i < changes + 1; i++) {
                if ((i % 2) == 0) {
                    legs.add(new Trip.Individual(Trip.Individual.Type.BIKE, from, Timestamp.fromDateAndNetworkOffset(new Date(42)), to, Timestamp.fromDateAndNetworkOffset(new Date(43)), null,
                            42));
                } else {
                    legs.add(new Trip.Public(dummyLine, null, departureStop, arrivalStop, null, null, null));
                }
